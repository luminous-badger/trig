...
# CoSine Z^2 fractal. Zoom in to RHS.
...
from dark_light import colour_list 
import sys
...
MsgText = 'Julia Fractal for Cos( Z^2 ) +' + str( C ) + ' and rnum: ' + str( rnum )

draw = ImageDraw.Draw(img)
font = ImageFont.truetype( "/Library/Fonts/Arial.ttf", 12 )
draw.text( ( 0, 0 ),  MsgText, ( 139,0,0 ), font=font )

print 'Julia Fractal for Cos( Z^2 ) ', C, 'and Rand:', rnum, 'created in %f s' % dt
print 'Calc: ', calc_count

fname = 'Julia_Cos_Zsq:' + str( C ) + '_X:' + str( X_MAX ) + str( X_MIN ) + '_Y:' + str( Y_MAX ) + str( Y_MIN ) + '.png'
print 'Fname:', fname
